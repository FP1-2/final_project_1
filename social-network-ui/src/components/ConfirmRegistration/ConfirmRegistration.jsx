  ...
  return (
    <div className={style.messageWrapper}>
      {statusSubmit === "fulfilled" ?
        <>
          <p className={style.message}>The user whose email is <span
            className={style.email}>{email}</span> has been successfully registered!</p>
          <NavLink to="/login" className={style.loginLink}>Log In</NavLink>
        </>
        : errorSubmit === "Invalid token or user not found" ?
          <p className={style.message}>{errorSubmit}</p> : null}
    </div>
  );
};
