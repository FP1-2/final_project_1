...
export default function ChatBody({ id, chat, isHovered, messages, messagesList, authUser, newMess, handleMouseEnter, hasMore, handleLoadMoreMessages,handleOpenImg }) {
    const isCreatingNewChat = id !== "new";
    return (
        isCreatingNewChat &&
            <div className={`${messagesList.length === 0 ? styles.chat__emptyList : styles.chat__body}`} onMouseEnter={handleMouseEnter}>
                {messagesList.length === 0 && (
                    messages.status === 'fulfilled' && (
                        <>
                            ...
                            />
                            <p className={styles.chat__emptyList__name}>{chat.obj.chatParticipant.name} {chat.obj.chatParticipant.surname}</p>
